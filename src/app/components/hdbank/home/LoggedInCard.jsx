...
const LoggedInCard = ({ mainPackages, subPackages, isLoading }) => {
  ...
  useEffect(() => {
    if (!cartId) return;
    subscriberService.getAutobill(cartId).then((res) => {
      if (res.success && res.data?.auto_bill) setAutoBill(res.data?.auto_bill);
    });
  }, [cartId]);

  const isAutoBillActive = autoBill?.status === "ACTIVE";
