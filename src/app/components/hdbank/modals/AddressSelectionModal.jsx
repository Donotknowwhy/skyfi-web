...
const AddressSelectionModal = () => {
	const t = useTranslations('hdbank.addressModal');
		...
		<div className="w-full bg-white rounded-t-2xl max-h-[90vh] flex flex-col">
			...
			<div className="px-4 pb-4">
				<div className="flex items-center justify-between mb-4">
					...
					<button
						onClick={handleReset}
						className="text-sm text-[#DA2128] font-semibold hover:underline"
					>
						{t('reset')}
					</button>
				</div>
				...
				<div className="flex flex-col space-y-2">
					...
					<div className="flex items-center space-x-4">
						<div className="flex items-center space-x-2">
							<div className={`w-2 h-2 rounded-full ${selectedCity ? 'bg-[#DA2128]' : 'bg-gray-300'}`}></div>
							{selectedCity && step > 1 && (
								<div className="w-0 h-6 border-l border-[#DA2128]"></div>
							)}
						</div>
						<span className={`text-sm font-inter ${selectedCity ? 'text-[#222222] font-medium' : 'text-[#DA2128]'}`}>
					...
					{selectedCity && (
						<div className="flex items-center space-x-4">
							<div className="flex items-center space-x-2">
								<div className={`w-2 h-2 rounded-full ${selectedDistrict ? 'bg-[#DA2128]' : step === 2 ? 'border-2 border-[#DA2128] bg-white' : 'bg-gray-300'}`}></div>
								{selectedDistrict && step > 2 && (
									<div className="w-0 h-6 border-l border-[#DA2128]"></div>
								)}
							</div>
							<span className={`text-sm font-inter ${selectedDistrict ? 'text-[#222222] font-medium' : 'text-[#DA2128]'}`}>
					...
					{selectedDistrict && (
						<div className="flex items-center space-x-4">
							<div className="flex items-center space-x-2">
								<div className={`w-2 h-2 rounded-full ${selectedWard ? 'bg-[#DA2128]' : step === 3 ? 'border-2 border-[#DA2128] bg-white' : 'bg-gray-300'}`}></div>
							</div>
							<span className={`text-sm font-inter ${selectedWard ? 'text-[#222222] font-medium' : 'text-[#DA2128]'}`}>
			...
			<div className="p-4 bg-white border-t border-[#F1F1F1]">
				{/* <button
					...
					className={`w-full py-3 px-6 rounded-lg font-semibold font-inter transition-colors ${
						selectedWard
							? 'bg-[#DA2128] text-white hover:bg-[#B71C22]'
