...
import { showModalMessVikki } from "../../modals/modalMess";

const ModalNotMoney = ({ code }) => {
  const t = useTranslations("vikki.modalNotMoney");
  ...
  const sendMessage = (code) => {
    const message = {
      ...
      bill_id: code,
      bill_type: "package",
      url_callback: `${getCallbackBaseUrl()}/app-vikki?sessionId=${sessionId}&billId=${code}`,
    };
    console.info("Sending message to React Native WebView:", message);
