		...
		<div className="bg-white rounded-[16px] p-4 shadow-[0px_2px_8px_0px_rgba(0,0,0,0.04)] mb-3 sm:mb-[24px]">
			...
			<div className="flex flex-col gap-[12px]">
				...
				{ payments.map( ( item, index ) => (
					<label key={ index } className={ `flex items-center gap-3 p-[12px] rounded-[12px] cursor-pointer transition-all ${ item.paymentMethod === payment_method ? 'bg-[#F0F5FF] border border-[#0057FF]' : 'border border-[#E5E5E5] bg-white' }` }>
						<div className="relative w-[20px] h-[20px] block">
							...
							/>
							<div className={ `w-full h-full rounded-full border flex items-center justify-center absolute top-0 left-0 pointer-events-none peer-checked:border-[#0057FF] border-[#C7C7CC] bg-white` } />
							<div className="w-[10px] h-[10px] absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 peer-checked:bg-[#0057FF] bg-transparent rounded-full transition-colors" />
						</div>
