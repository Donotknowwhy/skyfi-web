...
const PhoneSwitcher = () => {
    const t = useTranslations('vikki.home.phoneSwitcher');
                        ...
                        <div className="py-1">
                            {availablePhones.map((phone, index) => (
                                <button
                                    ...
                                    className={`w-full px-4 py-2.5 text-left text-sm hover:bg-gray-50 transition-colors flex items-center justify-between ${phone === cartId ? 'bg-blue-50 text-[#0000FF]' : 'text-[#333333]'
                                        }`}
                                >
                                    <span className="font-medium">{phone}</span>
                                    {phone === cartId && (
                                        <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                                            <path d="M13.3333 4L6 11.3333L2.66667 8" stroke="#0000FF" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                                        </svg>
                                    )}
                                </button>
                            ))}
