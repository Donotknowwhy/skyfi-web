...
const HotPackages = () => {
  const t = useTranslations("vikki.home.hotPackages");
  const [packages, setPackages] = useState([]);
  const [loading, setLoading] = useState(true);
  const { openCheckPackageModal } = usePackageViki();
  ...
  return (
    <div className="w-full mb-12" id="hot-packages">
      <div className="flex justify-between items-center mb-4 px-1">
        ...
        <Link
          href="/app-vikki/package"
          className="text-[#0057FF] text-[14px] font-bold leading-[20px]"
        >
          {t("explore")}
        </Link>
      </div>
