...
import { usePackageViki } from "@/app/hooks/usePackageViki";
import { toCurrency } from "@/app/utils/format";
import clsx from "clsx";
import { useTranslations } from "next-intl";
import { modal } from "@/app/utils/modal";
import PdfViewerContent from "../../modals/vikki/modalPdfViewer";

const PackageItem = ({ pack, isSelected, onSelect, count, total }) => {
  const { openPackDetailModal } = usePackageViki();
  const t = useTranslations("vikki.packageCard");
  ...
  return (
    <div
      className={`relative rounded-[16px] border cursor-pointer overflow-hidden transition-all ${isSelected ? "border-[#0057FF] bg-[#F0F5FF]" : "border-[#E5E5E5] bg-white"}`}
    ...
    >
      <div
        className={clsx(
          ...
          isVikki
            ? "border-[#D2008C] bg-[linear-gradient(88.19deg,#2C4EFF_-4.19%,#0000FF_12.9%,#6100FF_48.11%,#DA0191_74.13%,#FF8A00_89.99%,#FFB907_99.38%)]"
            : "",
        )}
      >
        <div className="flex items-center gap-1">
          ...
          {/* {hasDiscount && (
            <span className="bg-[#D2008C] text-white text-[12px] font-medium px-1.5 py-0.5 rounded">
              -{discountPercent}%
            </span>
          )} */}
        </div>
        ...
        {isVikki ? (
          <button
            ...
            className=" font-semibold text-[#D2008C] underline text-sm"
          >
            <svg
              width="20"
              height="20"
          ...
          <>
            ...
            <p className="text-[14px] text-[#5C5C5C] ">
              ...
              <a
                ...
                className="text-[#D2008C] underline cursor-pointer"
              >
                {t("giftNoteRules")}
              </a>
            </p>
      ...
      <div className="mt-3 pt-3 border-t border-gray-100 flex items-baseline justify-between gap-2 p-4">
        <div>
          <div className="flex items-baseline gap-1">
            <span className="text-[18px] font-bold text-[#D2008C]">
