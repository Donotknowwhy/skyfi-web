...
import TravelSimService from '@/app/services/travelsimM2M';
...
export default function TravelSimPage() {
    ...
    const onSubmit = ( data ) => {
        ...
        if ( type == 'freemium' ) {
            const link = `/travelsim-m2m/${ data.bookingCode }?type=${ type }`;
    ...
    const checkRecivedSkyboss = async ( code ) => {
        try {
            ...
            if ( msisdnData && msisdnData.received_sim == 1 ) {
                link = `/travelsim-m2m/confirm?orderId=${ code }`;
            }
            link = link || `/travelsim-m2m/${ code }?type=skyboss`;
            router.push( link );
        } catch ( error ) {
            showModalMess( {
                label: tErrors( 'title' ),
