...
export default function HDBankTopupPage() {
  const t = useTranslations('hdbank.topup');
  ...
  useEffect(() => {
    trackPageView({
      ...
      page_title: "Nạp tiền - HDBank Topup",
  ...
  const handleTopUp = async (data) => {
    try {
      ...
      if (response.success) {
        const paymentParams = applyPaymentSourceType(ORDER_BRANDS.HDBANK, {
        ...
        <form onSubmit={handleSubmit(handleTopUp)} className="px-4 pt-4">
          ...
          <div className="bg-white rounded-xl shadow-sm mb-4">
            <div className="flex items-center gap-3 p-4">
              {/* SkyFi Logo */}
              <div className="w-[41px] h-[41px] bg-[#DA2128] rounded-xl flex items-center justify-center flex-shrink-0">
              ...
              <div className="flex-1 border-b border-[#F1F1F1] pb-1">
                <label className="block text-sm text-[#5C5C5C] mb-1 font-normal">
                  {t('phoneNumber')}
                </label>
                <input
                  type="tel"
                    ...
                    topupValues.map((option) => (
                      <button
                        ...
                        className={`relative flex items-center justify-center px-4 py-3 rounded-lg border transition-all min-w-[165.5px] overflow-hidden ${
                          value === option.id
                            ? "border-[#DA2128] bg-white"
                            : "border-[#DDDDDD] bg-white hover:border-[#DA2128]"
                        }`}
                      >
                        ...
                        {value === option.id && (
                          <div className="absolute top-0 right-0 w-6 h-[25px] flex items-start justify-end ">
                            <div className="relative">
                              <div className="absolute top-0 right-0 w-7 h-[27px]  bg-[#DA2128]" style={{ clipPath: 'polygon(0 0, 100% 100%, 100% 0)' }}></div>
