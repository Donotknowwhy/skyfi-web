...
export default function BusinessCustomerSignaturePage() {
    ...
    const { locale, id, code } = params;
    const [customerInfo, setCustomerInfo] = useState(null);

    const handleViewInformation = async () => {
        setIsLoading(true);
        try {
            const res = await businessCustomerService.getCustomerInfo(id, code);
            // setCustomerInfo(res);

            // 👉 Mở modal hiển thị thông tin (tham khảo CustomerSignaturePage)
            modal.open({
                render: (
                ...
                closeButton: true,
                boxClassName: 'max-w-[820px] w-full'
            });
    ...
    const handleConfirm = async () => {
        ...
        try {
            ...
            const res = await businessCustomerService.signCustomer(
            ...
            <div className="bg-white rounded-lg shadow-sm p-6">
                <div className="mb-6">
                    ...
                    <p className="text-gray-600 text-sm">
                        Lưu ý: Bạn không thể tự thay đổi chữ ký sau khi đã xác nhận. Nếu muốn thay chữ ký, bạn phải liên hệ CSKH của Skyfi để xóa chữ ký hiện tại.
                    </p>
                </div>
