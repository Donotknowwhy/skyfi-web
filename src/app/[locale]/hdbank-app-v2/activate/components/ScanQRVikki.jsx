...
import { showModalConsentHDBank } from '@/app/components/modals/modalConsentHDBank';
...
const ScanQRVikki = () => {
  ...
  const requireConsent = pathname.includes('/hdbank-app-v2');
  const [error, setError] = useState(null);
  // hdbank-app-v2 gates the camera behind a consent popup; other variants open it directly
  const [isScanning, setIsScanning] = useState(!requireConsent);
  const fileInputRef = useRef(null);
  const { setValue } = useFormContext();
  const { open, close } = useLoad();

  // Require data-sharing consent before opening the camera (hdbank-app-v2 only)
  useEffect(() => {
    if (!requireConsent) return;
    showModalConsentHDBank({
      onConfirm: () => {
        setIsScanning(true);
      },
      onDismiss: () => {
