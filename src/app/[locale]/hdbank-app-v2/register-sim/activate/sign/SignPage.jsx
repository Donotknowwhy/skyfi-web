...
const SignPage = () => {
  ...
  const handleContinue = async () => {
    ...
    try {
      ...
      if (response?.success) {
        ...
        router.push(
          `/hdbank-app-v2/register-sim/activate/success?${params.toString()}`,
        );
      } else {
        showModalMessHDBank({
          label: "Thông báo",
  ...
  return (
    <div className="min-h-[100dvh] flex flex-col bg-white">
      <StepProgressHeader totalSteps={4} currentStep={4} />
      ...
      <div className="fixed bottom-0 left-0 right-0 z-40 flex gap-3 bg-white px-4 pb-8 pt-4 shadow-[0px_-4px_10px_0px_rgba(0,0,0,0.05)]">
        <button
          type="button"
          onClick={() => router.back()}
          className="flex-1 rounded-full bg-[#EDEDED] py-4 text-center text-lg font-bold text-[#1C1C1E]"
        >
          {t("back")}
        </button>
        <button
          ...
          className={`flex-1 rounded-full py-4 text-center text-lg font-bold transition-colors ${
            canContinue ? "text-white" : "bg-[#EDEDED] text-[#9CA3AF]"
          }`}
          style={
            canContinue
