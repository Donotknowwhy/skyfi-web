...
import RegisterNavBar from "@/app/[locale]/hdbank-app-v2/register-sim/components/RegisterNavBar";
...
const CheckInfoPage = () => {
  ...
  const fetchSessionData = async (currentSessionId, giftPhone = "") => {
    try {
      ...
      if (!ekycData?.chip_raw?.sod) {
        setMissingSod(true);
        showDownloadSkyFiModal(() => router.push("/hdbank-app-v2"));
      }
    ...
    try {
      ...
      if (typeof window !== "undefined") {
        const message = {
          ...
          user_id: userVikki || "",
          session_id: currentSessionId,
          url_callback: `${getCallbackBaseUrl()}/hdbank-app-v2/register-sim/activate/check-info?page=infoActivate&sessionId=${currentSessionId}`,
        };
  ...
  const handleContinue = () => {
    ...
    router.push(
      `/hdbank-app-v2/register-sim/activate/sign?${params.toString()}`,
    );
  };
