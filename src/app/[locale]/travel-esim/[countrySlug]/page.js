...
import {trackPageView, trackProductListView, trackProductView, trackAddToCart, trackBeginCheckout} from '@/app/utils/trackingHelper';
...
export default function CountryEsimPage() {
  ...
  useEffect(() => {
    trackPageView({ page_title: `Travel eSIM - ${countrySlug}` });
  }, [countrySlug]);
      ...
      try {
        ...
        if (packagesResponse.data && packagesResponse.data.code === 200 && Array.isArray(packagesResponse.data.result)) {
          ...
          trackProductListView({
            ...
            filters: { country_code: countrySlug, country_name: foundCountry.name },
          });
          if (packages.length > 0) {
            trackProductView({
              product_id: packages[0].id,
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-[16px] md:gap-[24px]">
                  {esimPackages.map((pkg) => (
                    ...
                    >
                      ...
                      <button
                        className="mt-6 w-full bg-primary hover:bg-[#E0910A] text-white font-semibold py-3 px-4 rounded-lg transition-colors duration-300 focus:outline-none focus:ring-2 focus:ring-primary focus:ring-opacity-50"
                        onClick={() => {
                          trackAddToCart({
                            product_id: pkg.id,
                            product_name: pkg.name,
                            product_category: `eSIM/${countryDetails?.name}`,
                            product_price: pkg.selling_price,
                            product_quantity: 1,
                            currency: pkg.currency || 'USD',
                          }, {
                            event_params: { country_code: countrySlug, button_location: 'country_esim_page', added_from: 'buy_now_button' },
                          });
                          trackBeginCheckout({
                            cart_total: pkg.selling_price,
                            currency: pkg.currency || 'USD',
                            items: [{ product_id: pkg.id, product_name: pkg.name, quantity: 1, price: pkg.selling_price }],
                          });
                        }}
                      >
                        {t('buyNowButton')}
