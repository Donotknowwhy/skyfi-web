...
const InputActivateVikki = () => {
  const t = useTranslations("vikki.inputActivate");
  const { onPush } = useActivateVikki();
  const {
    setValue,
    register,
    formState: { errors },
  ...
  const sendMessage = async (sessionId, imsi) => {
    ...
    try {
      ...
      await dkttService.updateSimInfo({
      ...
      if (typeof window !== "undefined") {
        const message = {
          ...
          user_id: userVikki || "",
          session_id: sessionId,
          url_callback: `${getCallbackBaseUrl()}/app-vikki/activate?page=infoActivate&sessionId=${sessionId}`,
        };
  ...
  // handle back
  const handleBack = () => {
    router.replace(`/app-vikki?sessionId=${sessionId}`);
  };
