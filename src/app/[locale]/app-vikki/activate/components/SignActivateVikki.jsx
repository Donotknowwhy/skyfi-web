...
import { showModalIframeVikki } from '@/app/components/modals/vikki/ModalIframeVikki';
...
import { ORDER_BRANDS, applyActivateVideoSource } from '@/app/utils/orderSourceContext';
...
// Sinh transId cục bộ dạng yyMMddHHmmss-8-4-4-4-12 hex, giống hàm
// generateTransId trong registerMsisdnNfc của app SkyFi (Flutter)
const generateTransId = () => {
  const now = new Date();
  const p = (n) => String(n).padStart(2, '0');
  const ts = `${String(now.getFullYear()).slice(2)}${p(now.getMonth() + 1)}${p(
    now.getDate(),
  ...
  return `${ts}-${hex(8)}-${hex(4)}-${hex(4)}-${hex(4)}-${hex(12)}`;
};

const SignActivateVikki = () => {
  const t = useTranslations('vikki.signActivate');
  const tn = useTranslations('vikki.activateProvider.notifications');
  const [isAgreed, setIsAgreed] = useState(false);
  const [hasSignature, setHasSignature] = useState(false);
  const sigCanvas = useRef(null);
  const { handleSubmit, setValue, getValues } = useFormContext();
  const { onBack, getContact } = useActivateVikki();
  ...
  const handleContinue = async () => {
    if (!isAgreed) return showModalMessVikki({
      label: t('notification'),
      message: t('pleaseAgree'),
      type: 'error',
    });
    if (!hasSignature) return showModalMessVikki({
      label: t('notification'),
    ...
    const ekycData = getValues('ekycRaw') || {};
    const chipRaw = ekycData.chip_raw || {};

    // Signature stored as a full data URL by handleEnd — strip the prefix
    const signature = (getValues('data.img4') || '').replace(
      /^data:image\/png;base64,/,
      '',
    );

    // NFC data gửi dạng HEX giống app SkyFi (Flutter đọc chip ra hex);
    // chip_raw từ eKYC session là base64 nên phải convert (data.sob đã
    // được hex hoá sẵn ở InfoActivateVikki)
    const nfcCom = base64ToHex(chipRaw.com || '');
    const nfcSod = data.sob || base64ToHex(chipRaw.sod || '');
    const nfcDg15 = base64ToHex(chipRaw.dg15 || '');

    const img1 = ekycData.img_front || data.img1 || '';
    const img2 = ekycData.img_back || ekycData.img_front || data.img2 || '';
    const img3 = ekycData.img_portrait || data.img3 || '';

    try {
      open();

      // B1: get_img4_app — đổi chữ ký base64 sang URL + lấy mã hợp đồng
      // (mahd), giống registerMsisdnNfc bên app SkyFi (Flutter)
      let img4Url = '';
      let mahd = '';
      try {
        const {
        ...
        const img4Res = await dkttService.getContact({
          ...logData,
          img1,
          img2,
          img3,
        ...
        img4Url = img4Res?.extra?.img || '';
        mahd = img4Res?.extra?.mahd || '';
      } catch (e) {
        console.error('Error fetching img4/mahd:', e);
      }

      // B2: view-cam-ket-dvvt — lấy URL "Phiếu cam kết DVVT" (bbck).
      // Lỗi KHÔNG chặn luồng đăng ký, giống Flutter.
      let bbck = '';
      try {
        const camKetRes = await dkttService.viewCamKetDvvt({
          full_name: data.fullName || '',
          id_number: data.idNumber || '',
          place_date: data.issueDate || '',
          place_of_issue: data.issuePlace || '',
          dob: data.birthDay || '',
          so_tb: data.phone || '',
          kh_name: data.fullName || '',
          code: '',
          kh_sign: signature,
        });
        bbck = camKetRes?.data?.url || '';
      } catch (e) {
        console.error('Error fetching cam ket dvvt:', e);
      }

      // Build the /app/register-msisdn-nfc payload (same shape as the SkyFi
      // Flutter app). videoSource marks this as a Vikki registration.
      const param = applyActivateVideoSource(ORDER_BRANDS.VIKKI, {
        face_enroll: {
          faceImages: [img3],
      ...
      if (response?.success) {
        return showModalMessVikki({
          title: tn('title'),
          message: tn('registerSuccess'),
          labelConfirm: tn('goHome'),
          onConfirm: () => {
            router.push(`/app-vikki?sessionId=${sessionId}`);
          },
        });
      }
      ...
      showModalMessVikki({
        label: t('notification'),
        message: response?.message || tn('registerFailed'),
        type: 'error',
      });
    } catch (error) {
  ...
  return (
    <div
      ...
      style={{ backgroundImage: 'url(/images/vikki/bg-activate.png)' }}
    >
      {/* Header */}
        ...
        <div className="flex flex-col gap-6 p-4 bg-white/80 rounded-xl border border-[rgba(84,85,86,0.12)] shadow-[0px_1px_1px_0px_rgba(0,0,0,0.03),0px_0px_1px_0px_rgba(0,0,0,0.15),0px_4px_16px_0px_rgba(144,118,170,0.08)]">
          ...
          <div className="flex flex-col gap-3">
            ...
            <label className="flex items-center gap-2 cursor-pointer">
              <div
                className={`w-4 h-4 rounded flex items-center justify-center ${isAgreed ? 'bg-[#0000EA]' : 'border border-gray-300'
                  }`}
            ...
            <ul className="flex flex-col gap-2 pl-1 text-xs text-[#333333] leading-[1.5]">
              ...
              <li className="flex gap-1">
                ...
                <button
                  type="button"
                  onClick={() => showModalIframeVikki({ url: '/personal-data-protection-policy?src=app' })}
              ...
              <li className="flex gap-1">
                ...
                <button
                  type="button"
                  onClick={() => showModalIframeVikki({ url: '/refund-policy?src=app' })}
                  className="text-[#C4107C] underline font-bold"
                >
                  {t('returnPolicy')}
                </button>
