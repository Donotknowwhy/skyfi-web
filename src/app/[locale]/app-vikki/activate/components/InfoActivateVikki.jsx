...
import VikkiService from "@/app/services/vikkiService";
...
const InfoActivateVikki = () => {
  const t = useTranslations("vikki.infoActivate");
  const { setValue, watch, handleSubmit, getValues } = useFormContext();
  const { onBack, onPush } = useActivateVikki();
  const router = useRouter();
  const [loading, setLoading] = useState(false);
  // eKYC without NFC sod cannot register via this flow — direct to the SkyFi app
  const [missingSod, setMissingSod] = useState(false);

  const sessionId = watch("sessionId");
  const userInfo = watch("data") || {};

  const showDownloadSkyFiModal = () =>
    showModalMessVikki({
      label: "Thông báo",
      message:
      ...
      type: "info",
      labelConfirm: "Đóng",
      onConfirm: () => router.push(`/app-vikki?sessionId=${sessionId}`),
    });
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // if (ekycData) {
        //   try {
        //     const convertResult = await VikkiService.convertS3ToBase64({
        ...
        // Keep the raw eKYC response so the sign step can build the
        // register-msisdn-nfc payload (needs chip_raw, trans_id, province, ...)
        setValue("ekycRaw", ekycData || {});

        // No NFC sod → registration will fail; block and send user to SkyFi app
        if (!ekycData?.chip_raw?.sod) {
          setMissingSod(true);
          showDownloadSkyFiModal();
        }
    ...
    >
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white rounded-t-2xl shadow-[0px_0px_16px_0px_rgba(0,0,0,0.1)] px-4 py-4 pb-8">
        <button
          ...
          style={{
            background:
              "linear-gradient(39deg, rgba(44, 78, 255, 1) 0%, rgba(0, 0, 255, 1) 4%, rgba(97, 0, 255, 1) 47%, rgba(218, 1, 145, 1) 78%, rgba(255, 138, 0, 1) 98%, rgba(255, 185, 7, 1) 100%)",
          }}
        >
          {t("continueButton")}
        </button>
          ...
          render={({ field, fieldState: { error } }) => (
            <>
              <input
                ...
                className={`w-full text-base font-medium text-[#333333] leading-6 bg-white border rounded-lg px-3 py-2 outline-none focus:border-[#0000EA] transition-colors ${
