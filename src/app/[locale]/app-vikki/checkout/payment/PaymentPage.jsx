...
import { showAlertVikki } from '@/app/components/modals/vikki/modalAlertVikki';
import { Button } from '@/app/components/ui/Button';
import Delivery from '@/app/components/vikki/checkout/Delivery';
import InfoClient from '@/app/components/vikki/checkout/InfoClient';
import ListProduct from '@/app/components/vikki/checkout/ListProduct';
import SummaryCheckout from '@/app/components/vikki/checkout/SummaryCheckout';
...
import { validateVikkiCheckout } from '@/app/utils/validate';
...
export default function PaymentPage() {
    const t = useTranslations('vikki.checkout.payment');
    ...
    const method = useForm({
        resolver: yupResolver(validateVikkiCheckout)
    });
    ...
    useEffect(() => {
        ...
        method.setValue('source', getOrderSourceContext(ORDER_BRANDS.VIKKI).appSource);
    ...
    const renderTermsText = () => {
        return (
            <>
                ...
                <span onClick={showDevicesEsimVikki}
                    className="text-[#D2008C] cursor-pointer underline font-semibold">{t('esimModal')}</span>
                <span> {t('agreeTermsPrefix')} </span>
                <span onClick={handleTermsClick}
                    className="text-[#D2008C] cursor-pointer underline font-semibold"
                >
                    {t('agreeTerms')}
                </span>
            </>
    ...
    const renderTermsCheckbox = (id, checked, onChange, label) => (
        <div className="flex items-start gap-3 mb-3" >
            <div
                onClick={onChange}
                className={`w-5 h-5 rounded flex items-center justify-center border ${checked ? 'bg-[#0057FF] border-[#0057FF]' : 'border-[#C7C7CC] bg-white'}`}>
                {checked && (
            ...
            <label htmlFor={id} className="text-[14px] text-[#333] leading-[20px] select-none cursor-pointer flex-1" >
                {label}
            </label>
        </div>
    );
    ...
    const sendMessage = (code) => {
        if (typeof window !== 'undefined') {
            const message = {
                ...
                "bill_id": code,
                "bill_type": "online",
                "url_callback": `${getCallbackBaseUrl()}/app-vikki/checkout/result?orderId=${code}`
            }
    ...
    const handleSubmit = async (data) => {
        try {
            ...
            if (res.order_number && res.total_amount === 0) {
                ...
                const orderDetailUrl = `/app-vikki/checkout/result?orderId=${res.order_number}`;
                setTimeout(() => {
        ...
        return (
            <div className="bg-[#F5F5F5] font-koho min-h-screen flex flex-col">
                ...
                <div className="flex-1 flex flex-col items-center justify-center p-4">
                    ...
                    <Button
                        onClick={() => router.push('/vikki-app/sim-data')}
